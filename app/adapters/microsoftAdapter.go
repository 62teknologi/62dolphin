...
func (adp *MicrosoftAdapter) Verify(ctx *gin.Context) (map[string]any, error) {
	...
	utils.DB.Table("users").Where("email = ?", req.Email).Where("microsoft_id = ?", req.UserId).Take(&user)
